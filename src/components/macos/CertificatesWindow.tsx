...
const CertificatesWindow = ({ isDark = false }: { isDark?: boolean }) => {
  ...
  return (
    <div className="flex flex-col h-[65vh] relative overflow-hidden">
      ...
      {/* Lightbox Gallery - Sibling to Scrollable Area */}
      {lightboxIndex !== null && (
        <div
        ...
        >
          ...
          <div className="w-full flex-1 min-h-0 flex items-center justify-center mt-12 mb-4 px-4 sm:px-12" onClick={(e) => e.stopPropagation()}>
            <img
              ...
              className="max-w-4xl max-h-full object-contain bg-white/10 backdrop-blur-3xl rounded-[2rem] border border-white/20 shadow-2xl p-2 sm:p-4"
            />
          </div>
